...
from models.processing.preparation import separate_seqs, remove_nan_values_from_tensors, separate_seqs_all, separate_seqs_all_middle_frame_all, shuffle_data, plot_pyr_graphs
...
MODEL_LSTM_CLASS_FILENAME = 'lstm_class_bi_windows_middle_shake.h5'
MASK_VALUE = [-10]*4
PAD_VALUE = [0]*4
...
def fit_lstms_class_windows_middle_shake(df_train, df_val, model_dir, overwrite=True, debug=False, epochs=100, window_size=36, load_values=False):
    ...
    if load_values and (Path(model_dir)/'train_x_windows_middle_shake.npy').exists() and (Path(model_dir)/'val_x_windows_middle_shake.npy').exists()\
                   and (Path(model_dir)/'train_y_windows_middle_shake.npy').exists() and (Path(model_dir)/'val_y_windows_middle_shake.npy').exists():
        print('Loading train and val data from pickle')
        train_x, train_y, val_x, val_y = np.load(Path(model_dir)/'train_x_windows_middle_shake.npy'), np.load(Path(model_dir)/'train_y_windows_middle_shake.npy'), \
                np.load(Path(model_dir)/'val_x_windows_middle_shake.npy'), np.load(Path(model_dir)/'val_y_windows_middle_shake.npy')
    else:
        ...
        print("Removing files that do not contain shakes")
        indexes_with_shakes = [index for index, array in enumerate(train_y) if 1 in array]
        print("Nr of files in training data that contain shakes: ", len(indexes_with_shakes), "/", len(train_y), ", namely the indexes: ", indexes_with_shakes)
        train_y = [train_y[index] for index in indexes_with_shakes]
        train_x = [train_x[index] for index in indexes_with_shakes]
        ...
        train_background_vectors += train_nod_vectors
        train_background_labels = [0]*len(train_background_vectors)
        train_shake_labels = [1]*len(train_shake_vectors)

        print('Plotting some graphs')
        plot_pyr_graphs(train_shake_vectors, number=1, add_diff = False, diff_pix=False, diff_ang=False, movement_type='Shake', ang_norm=False)
        ...
        print("Removing files that do not contain shakes")
        indexes_with_shakes = [index for index, array in enumerate(val_y) if 1 in array]
        print("Nr of files in validation data that contain shakes: ", len(indexes_with_shakes), "/", len(val_y), ", namely the indexes: ", indexes_with_shakes)
        val_y = [val_y[index] for index in indexes_with_shakes]
        val_x = [val_x[index] for index in indexes_with_shakes]
        ...
        val_background_vectors += val_nod_vectors
        val_background_labels = [0]*len(val_background_vectors)
        val_shake_labels = [1]*len(val_shake_vectors)
        
        # print('Augmenting data')
        # train_shake_vectors, _ = augment_flip_data(train_shake_vectors, [2]*len(train_shake_vectors))
        # val_shake_vectors, _ = augment_flip_data(val_shake_vectors, [2]*len(val_shake_vectors))

        print('Check number of data points: ')
        min_train, min_val = len(train_shake_vectors), len(val_shake_vectors)
        ...
        print('Concatenate')
        train_x = np.array(train_background_vectors + train_shake_vectors)
        train_y = np.array(train_background_labels + train_shake_labels)
        val_x = np.array(val_background_vectors + val_shake_vectors)
        val_y = np.array(val_background_labels + val_shake_labels)
        ...
        joblib.dump(scalers, Path(model_dir)/'scalers_middle_shake.pkl')

        np.save(Path(model_dir) / Path(f'train_x_windows_middle_shake.npy'), train_x)
        np.save(Path(model_dir) / Path(f'train_y_windows_middle_shake.npy'), train_y)
        np.save(Path(model_dir) / Path(f'val_x_windows_middle_shake.npy'), val_x)
        np.save(Path(model_dir) / Path(f'val_y_windows_middle_shake.npy'), val_y)
    ...
    simple_sign_dict = {'background': 0, 'shake': 1}
    ...
    callbacks1 = ModelCheckpoint('data/class_windows_middle_shake.hdf5', save_best_only=True)
...
if __name__ == '__main__':
    ...
    fit_lstms_class_windows_middle_shake(args.frames_csv, args.model_dir, args.overwrite)
