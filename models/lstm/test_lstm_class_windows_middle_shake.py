...
from models.lstm.train_lstm_class_windows_middle_shake import MODEL_LSTM_CLASS_FILENAME, PAD_VALUE, MASK_VALUE, plot_pyr_graphs
...
def predict_lstm_class_windows_middle_shake(df_test, models_dir, window_size=36):
    ...
    indexes_with_shakes = [index for index, array in enumerate(labels) if 1 in array]
    print("Nr of files in validation data that contain shakes: ", len(indexes_with_shakes), "/", len(labels), ", namely the indexes: ", indexes_with_shakes)
    labels = [labels[index] for index in indexes_with_shakes]
    vectors = [vectors[index] for index in indexes_with_shakes]
    ...
    for vector_i, vector in enumerate(vectors):
        ...
        if not (Path(models_dir)/'scalers_middle_shake.pkl').exists():
            print("No scaler found")
            return [], []

        scalers = joblib.load(Path(models_dir)/'scalers_middle_shake.pkl')
...
if __name__ == '__main__':
    ...
    predict_lstm_class_windows_middle_shake(args.frames_csv, args.models_dir, args.window_size)
